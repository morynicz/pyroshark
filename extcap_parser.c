...
gchar *extcap_get_complex_as_string(extcap_complex *comp) {
	...
	if (comp == NULL) {
		snprintf(ret, 32, "(null)");
		return ret;
	}

	switch (comp->complex_type) {
	case EXTCAP_ARG_INTEGER:
		snprintf(ret, 32, "%d", comp->complex_value.int_value);
		break;
	case EXTCAP_ARG_UNSIGNED:
		snprintf(ret, 32, "%u", comp->complex_value.uint_value);
		break;
	case EXTCAP_ARG_LONG:
		snprintf(ret, 32, "%ld", comp->complex_value.long_value);
		break;
	case EXTCAP_ARG_DOUBLE:
		snprintf(ret, 32, "%f", comp->complex_value.double_value);
		break;
...
extcap_arg *extcap_new_arg(void) {
	...
	r->call = r->display = r->tooltip = NULL;
	r->arg_type = EXTCAP_ARG_UNKNOWN;
	r->range_start = r->range_end = NULL;
...
void extcap_free_arg_list_cb(gpointer listentry)
{
	if ( listentry != NULL )
...
gint glist_find_numbered_arg(gconstpointer listelem, gconstpointer needle)
{
	if (((extcap_arg *)listelem)->arg_num == *((int*)needle))
		return 0;
	return 1;
}
...
extcap_arg *extcap_parse_arg_sentence(GList * args, extcap_token_sentence *s) {
	...
	if (sent == EXTCAP_SENTENCE_ARG) {
		...
		if ((v = extcap_find_param_by_type(s->param_list, EXTCAP_PARAM_RANGE))
			...
			if ((target_arg->range_start = extcap_parse_complex(target_arg->arg_type,
					v->value)) == NULL) {
				printf("invalid range, expected value,value got %s\n",
						v->value);
				extcap_free_arg(target_arg);
			...
			if ((target_arg->range_end = extcap_parse_complex(target_arg->arg_type,
					cp + 1)) == NULL) {
				printf("invalid range, expected value,value got %s\n",
						v->value);
				extcap_free_arg(target_arg);
		...
		if ((v = extcap_find_param_by_type(s->param_list, EXTCAP_PARAM_DEFAULT))
				!= NULL) {
			if ((target_arg->default_complex = extcap_parse_complex(target_arg->arg_type,
					v->value)) == NULL) {
		...
		;
		if ((entry = g_list_find_custom(args, &tint, glist_find_numbered_arg)) == NULL) {
		...
		((extcap_arg*)entry->data)->values = g_list_append(((extcap_arg*)entry->data)->values, value);
